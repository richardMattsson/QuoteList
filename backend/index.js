...
app.post("/api/post", async (request, response) => {
    ...
    const { rows } = await client.query("INSERT INTO quotes (name, quote) VALUES ($1, $2) RETURNING *", [name, quote]);
    response.send(rows);
});
app.put("/api/put/:id", async (request, response) => {
    const { name, quote } = request.body;
    const { id } = request.params;
    const { rows } = await client.query("UPDATE quotes set name = $1, quote = $2 WHERE id = $3 RETURNING *", [name, quote, id]);
    response.send(rows);
});
